...
$selectSql = "SELECT 
                ...
                WHERE cis.register_by = us.userid AND cis.ciid = ci.ciid) as registerId,
                (SELECT 
                    us.fname 
                FROM user us, chemicalin cis 
                WHERE cis.register_by = us.userid AND cis.ciid = ci.ciid) as registerFName,
                (SELECT 
                    us.lname 
                FROM user us, chemicalin cis 
                WHERE cis.register_by = us.userid AND cis.ciid = ci.ciid) as registerLName, 
                l.name 
...
$htmlcontent .=  '<tr>
					...
					<td>'.$row['registerId'].' '.$row['registerFName'].' '.$row['registerLName'].'</td>
					<td>'.$row['name'].'</td>
				  </tr>';
}
